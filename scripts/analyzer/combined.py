...
def restructure_and_add_function_info(combined_data, source_directory):
    ...
    for testcase in combined_data['mutmut_result']['testsuite'][0]['testcase']:
        ...
        for func_name, data in functions_data.items():
            start, end = map(int, data['function_scope'].split('-'))
    ...
    # Calculate mutation scores and finalize data structure
    for func_name, func_data in functions_data.items():
        func_data['mutation_score'] = calculate_mutation_score(func_data['mutants'])
